...
class CustomerController extends Controller {
	...
	public function __construct()
	{
		...
		$this->modelview = new  \App\Models\Invoicemain();
...
// master details form CRF token check point start here. CSRf token verification join both module invoice and customer.
	...
	public function getIndex( Request $request )
	{
		...
		$sort = (!is_null($request->input('sort')) ? $request->input('sort') : 'CustomerId'); 
	...
	{
	// master details form contron active here only view invoice details on customer table....
...
// For better work the form to saving data here the subgrid table function is hideen type..
		...
		if ($validator->passes()) {
			...
			$id = $this->model->insertRow($data , $request->input('CustomerId'));
	...
	public function postDelete( Request $request)
	{
		...
		if(count($request->input('ids')) >=1)
		{
			...
			\DB::table('invoice')->whereIn('customerid',$request->input('ids'))->delete();
