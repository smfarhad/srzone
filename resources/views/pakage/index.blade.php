...
<div class="sbox-tools" >
	...
	<div class="sbox-content"> 	
	    <div class="toolbar-line ">
			@if($access['is_add'] ==1)
	   		<a href="{{ URL::to('pakage/update') }}" class="tips btn btn-lg btn-primary"  title="Add Pakage">
			<i class="fa fa-plus-circle "></i>&nbsp;Add New Pakage</a>
			@endif  
			@if($access['is_remove'] ==1)
			<a href="javascript://ajax"  onclick="SximoDelete();" class="tips btn btn-lg btn-primary" title="{{ Lang::get('core.btn_remove') }}">
			<i class="fa fa-minus-circle "></i>&nbsp;{{ Lang::get('core.btn_remove') }}</a>
			@endif 
			<a href="{{ URL::to( 'pakage/search') }}" class="tips btn btn-lg btn-primary" onclick="SximoModal(this.href,'Advance Search'); return false;" ><i class="fa fa-search"></i> Search</a>				
			@if($access['is_excel'] ==1)
			<a href="{{ URL::to('pakage/download?return='.$return) }}" class="tips btn btn-lg btn-primary" title="{{ Lang::get('core.btn_download') }}">
	 ...
	 {!! Form::open(array('url'=>'pakage/delete/', 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) !!}
    ...
    <table class="table table-striped ">
        ...
        <tbody>        						
            @foreach ($rowData as $row)
                <tr>
					<td width="30"> {{ ++$i }} </td>
					<td width="50"><input type="checkbox" class="ids" name="ids[]" value="{{ $row->PakageId }}" />  </td>									
				 ...
				 <td>
						...
						<a href="{{ URL::to('pakage/show/'.$row->PakageId.'?return='.$return)}}" class="tips btn btn-xs btn-primary" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search "></i></a>
						@endif
						@if($access['is_edit'] ==1)
						<a  href="{{ URL::to('pakage/update/'.$row->PakageId.'?return='.$return) }}" class="tips btn btn-xs btn-success" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit "></i></a>
						@endif
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#SximoTable').attr('action','{{ URL::to("pakage/multisearch")}}');
		$('#SximoTable').submit();
	});
