  ...
  <div class="page-content row">
    ...
    <div class="page-header">
      ...
      <ul class="breadcrumb">
		...
		<li><a href="{{ URL::to('payment?return='.$return) }}">{{ $pageTitle }}</a></li>
...
<div class="col-md-12">
					...
					$payment_method_opt = array( 'Cash' => 'Cash Payment' ,  'bKash' => 'bKash Payment' ,  'Others' => 'Others' , ); ?>
					<select name='payment_method' rows='5'   class='select2 '  > 
						...
						?></select> 
								  ...
								  <div class="form-group  " >
									<label for="Payment For" class=" control-label col-md-4 text-left"> Payment For </label>
									<div class="col-md-6">
									  
					<?php $payment_for = explode(',',$row['payment_for']);
					$payment_for_opt = array( 'Due' => 'Due Payment' ,  'Advance' => 'Advance Payment' ,  'Connection Fee' => 'Connection Fee' , ); ?>
					<select name='payment_for' rows='5'   class='select2 '  > 
						<?php 
						foreach($payment_for_opt as $key=>$val)
						{
							echo "<option  value ='$key' ".($row['payment_for'] == $key ? " selected='selected' " : '' ).">$val</option>"; 						
						}						
						?></select> 
									 </div> 
									 <div class="col-md-2">
...
<!-- 					<button type="submit" name="apply" class="btn btn-info btn-sm" ><i class="fa  fa-check-circle"></i> {{ Lang::get('core.sb_apply') }}</button> -->
					<button type="submit" name="submit" class="btn btn-primary btn-sm" ><i class="fa  fa-save "></i>&nbsp;PAY NOW</button>
					<button type="button" onclick="location.href='{{ URL::to('payment?return='.$return) }}' " class="btn btn-success btn-sm "><i class="fa  fa-arrow-circle-left "></i>  {{ Lang::get('core.sb_cancel') }} </button>
					</div>	  
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#CustomerId").jCombo("{{ URL::to('payment/comboselect?filter=customer:CustomerId:CustomerId|FullName') }}",
