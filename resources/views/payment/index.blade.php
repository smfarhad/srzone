...
<div class="sbox-tools" >
	...
	<div class="sbox-content"> 	
	    <div class="toolbar-line ">
			@if($access['is_add'] ==1)
	   		<a href="{{ URL::to('payment/update') }}" class="tips btn btn-lg btn-primary"  title="Add New Payment">
			<i class="fa fa-plus-circle "></i>&nbsp; NEW PAYMENT</a>
			@endif  
			@if($access['is_remove'] ==1)
			<a href="javascript://ajax"  onclick="SximoDelete();" class="tips btn btn-sm btn-white" title="{{ Lang::get('core.btn_remove') }}">
			<i class="fa fa-minus-circle "></i>&nbsp;{{ Lang::get('core.btn_remove') }}</a>
			@endif 
			<a href="{{ URL::to( 'payment/search') }}" class="tips btn btn-lg btn-primary" onclick="SximoModal(this.href,'Advance Search'); return false;" ><i class="fa fa-search"></i> Search</a>				
			@if($access['is_excel'] ==1)
			<a href="{{ URL::to('payment/download?return='.$return) }}" class="tips btn btn-sm btn-white" title="{{ Lang::get('core.btn_download') }}">
	 ...
	 {!! Form::open(array('url'=>'payment/delete/', 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) !!}
    ...
    <table class="table table-striped ">
        ...
        <tbody>        						
            @foreach ($rowData as $row)
                <tr>
					<td width="30"> {{ ++$i }} </td>
					<td width="50"><input type="checkbox" class="ids" name="ids[]" value="{{ $row->paymentid }}" />  </td>									
				 ...
				 <td>
						...
						<a href="{{ URL::to('payment/show/'.$row->paymentid.'?return='.$return)}}" class="tips btn btn-xs btn-primary" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search "></i> VIEW </a>
						@endif
						@if($access['is_edit'] ==1)
						<a  href="{{ URL::to('payment/update/'.$row->paymentid.'?return='.$return) }}" class="tips btn btn-xs btn-success" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit "></i></a>
						@endif
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#SximoTable').attr('action','{{ URL::to("payment/multisearch")}}');
		$('#SximoTable').submit();
	});
